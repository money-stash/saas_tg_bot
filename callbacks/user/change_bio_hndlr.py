...
@router.callback_query(F.data.startswith("change_bio:"))
async def start_update_bio(call: CallbackQuery, bot: Bot, state: FSMContext):
    await state.clear()
...
@router.message(F.text, ChangeBio.bio)
async def update_bio_hndlr(msg: Message, bot: Bot, state: FSMContext):
    ...
    if db.user_exists(all_users, user_id):
        ...
        if not new_name:
            await msg.answer("❌ Bio не может быть пустым. Попробуйте снова.")
            return
        ...
        session_info["bio"] = new_name
        is_updated = await update_bio(session_id, new_name)

        if is_updated["success"] is False:
            msg_text = "❌ Ошибка при обновлении bio сессии. Попробуйте позже."
        else:
            msg_text = f"✅ BIO сессии успешно обновлена на: {new_name}"
