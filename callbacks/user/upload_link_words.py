...
from keyboards.inline.user import get_cancel_menu, get_back_to_link
...
async def start_upload_link_words(
...
async def document_handler(message: Message, bot: Bot, state: FSMContext, user_id: int):
    if document := message.document:
        ...
        answer = await upload_file_to_link(
            link_id=data["link_id"], file_path=destination
        )
        if answer["success"] is True:
            await bot.edit_message_text(
                ...
                reply_markup=await get_back_to_link(data["link_id"]),
            )
        else:
            await bot.edit_message_text(
                chat_id=user_id,
